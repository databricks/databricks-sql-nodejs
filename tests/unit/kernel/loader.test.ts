...
import { KernelNativeLoader, KernelNativeBinding } from '../../../lib/kernel/KernelNativeLoader';

// Pure-logic tests for KernelNativeLoader. These exercise the load-failure
...
function stubBinding(overrides: Partial<Record<keyof KernelNativeBinding, unknown>> = {}): KernelNativeBinding {
  return {
    version: () => '1.2.3',
    openSession: async () => ({}),
...
describe('KernelNativeLoader', () => {
  describe('successful load', () => {
    it('get() returns the binding from the injected loader', () => {
      const binding = stubBinding();
      const loader = new KernelNativeLoader(() => binding, SUPPORTED_NODE_MAJOR);
    ...
    it('caches the result — the load function runs at most once', () => {
      let calls = 0;
      const binding = stubBinding();
      const loader = new KernelNativeLoader(() => {
        calls += 1;
        return binding;
  ...
  describe('load-failure hints', () => {
    it('MODULE_NOT_FOUND → "not installed" hint pointing at the README', () => {
      const loader = new KernelNativeLoader(() => {
        throw errWithCode('MODULE_NOT_FOUND', "Cannot find module '../../native/kernel'");
      }, SUPPORTED_NODE_MAJOR);
