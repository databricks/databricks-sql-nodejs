...
describe('KernelAuth — PAT auth options builder', () => {
  describe('buildKernelConnectionOptions', () => {
    ...
    it('rejects external-token, static-token, and custom auth modes', () => {
      ...
      for (const authType of authTypes) {
        ...
        expect(() => buildKernelConnectionOptions(opts)).to.throw(HiveDriverError, /unsupported auth mode/);
      }
    });
  });

  // Note: KernelBackend.connect/openSession round-trip + error-path coverage
  // moved to tests/unit/kernel/execution.test.ts during the kernel-integration
  // merge (the execution branch's KernelBackend constructor signature
  ...
  // tests/integration/kernel/auth-m2m-e2e.test.ts.
});
