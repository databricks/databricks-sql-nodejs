...
// `@databricks/databricks-sql-kernel-<triple>` (see native/kernel/README.md and the
// KernelNativeLoader load-failure hint).
describe('kernel native binding — packaging (native/kernel/index.js)', () => {
  ...
  it('every npm fallback uses the canonical @databricks/databricks-sql-kernel-<triple> name', () => {
    ...
    for (const name of required) {
      expect(name, `unexpected kernel native package name: ${name}`).to.match(triple);
    }
  });
