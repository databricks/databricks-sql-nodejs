 ...
 *
 * Verifies that the kernel path renders the exact thrift wire string for
 ...
 * `Int64` by `lib/kernel/KernelArrowIpcDurationFix.ts` because `apache-arrow@13`
 * predates the `Duration` type id).
 *
 * Reference failure modes (round 5 testing):
 *   - YEAR-MONTH:
 *       thrift  → `"1-2"`        (string)
 *       kernel pre-fix → `{"0":1,"1":2}` (Int32Array surfaced as struct)
 *   - DAY-TIME:
...
import { DURATION_UNIT_METADATA_KEY as REWRITER_DURATION_KEY } from '../../../lib/kernel/KernelArrowIpcDurationFix';
...
describe('KernelOperationBackend — INTERVAL parity with thrift', () => {
  it('YEAR-MONTH via native Arrow Interval[YearMonth] → "Y-M"', async () => {
    ...
    const backend = new KernelOperationBackend({ statement: stub, context: new ClientContextStub() });
  ...
  it('the duration_unit metadata key is identical in the rewriter and the converter', () => {
    ...
    // compile-time dependency on lib/kernel). Pin them equal so a rename/typo in
