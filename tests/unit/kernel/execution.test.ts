...
describe('KernelBackend', () => {
  ...
  it('openSession() forwards hostName / httpPath / token to napi binding', async () => {
    ...
    // sea-auth-u2m introduced the discriminated KernelNativeConnectionOptions
...
describe('KernelOperationBackend', () => {
  ...
  // the sea-results KernelResultsProvider + ArrowResultConverter pipeline
...
describe('KernelOperationBackend — async (submitStatement) path', () => {
  const makeAsyncOp = (asyncStatement: FakeAsyncStatement, queryTimeoutSecs?: number) =>
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    new KernelOperationBackend({ asyncStatement: asyncStatement as any, context: makeContext(), queryTimeoutSecs });
