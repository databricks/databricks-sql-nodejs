...
describe('SeaAuth TLS options (buildSeaTlsOptions)', () => {
  ...
  it('passes checkServerCertificateHostname through verbatim, independently of the master toggle', () => {
    expect(buildSeaTlsOptions(opts({ checkServerCertificateHostname: false }))).to.deep.equal({
      checkServerCertificateHostname: false,
    });
    // Independent of the master toggle — both can be set together.
    expect(
      buildSeaTlsOptions(opts({ checkServerCertificate: true, checkServerCertificateHostname: false })),
...
describe('SeaAuth mTLS options (buildSeaTlsOptions)', () => {
  it('emits no client identity by default', () => {
    const tls = buildSeaTlsOptions(opts({}));
  ...
  it('normalises string cert + key PEMs to Buffers', () => {
    const tls = buildSeaTlsOptions(opts({ clientCertPem: CERT_PEM, clientKeyPem: KEY_PEM }));
  ...
  it('passes Buffer cert + key through unchanged', () => {
    ...
    const tls = buildSeaTlsOptions(opts({ clientCertPem: cert, clientKeyPem: key }));
    expect(tls.clientCertPem).to.equal(cert);
    expect(tls.clientKeyPem).to.equal(key);
  });

  it('rejects supplying only the client cert', () => {
    expect(() => buildSeaTlsOptions(opts({ clientCertPem: CERT_PEM }))).to.throw(
      HiveDriverError,
  ...
  it('rejects supplying only the client key', () => {
    expect(() => buildSeaTlsOptions(opts({ clientKeyPem: KEY_PEM }))).to.throw(
      HiveDriverError,
      /requires both `clientCertPem` and `clientKeyPem`/,
    );
  });
  ...
  it('rejects a client cert that is not a PEM certificate', () => {
    expect(() => buildSeaTlsOptions(opts({ clientCertPem: 'nope', clientKeyPem: KEY_PEM }))).to.throw(
      HiveDriverError,
  ...
  it('rejects a client key that is not a PEM private key', () => {
    expect(() => buildSeaTlsOptions(opts({ clientCertPem: CERT_PEM, clientKeyPem: 'nope' }))).to.throw(
      HiveDriverError,
  ...
  it('rejects an empty cert Buffer', () => {
    expect(() => buildSeaTlsOptions(opts({ clientCertPem: Buffer.alloc(0), clientKeyPem: KEY_PEM }))).to.throw(
      HiveDriverError,
  ...
  it('folds mTLS into the full connection options', () => {
    const native = buildSeaConnectionOptions(opts({ clientCertPem: CERT_PEM, clientKeyPem: KEY_PEM })) as {
...
describe('SeaAuth HTTP options (buildSeaHttpOptions)', () => {
  ...
  it('passes caller customHeaders through, in order, with the connector User-Agent appended last', () => {
    const http = buildSeaHttpOptions(opts({ customHeaders: { 'X-Trace': 'abc', 'X-Env': 'prod' } }));
  ...
  it('drops kernel-managed reserved headers (Authorization / x-databricks-org-id, any casing)', () => {
    const http = buildSeaHttpOptions(
      opts({
        customHeaders: {
  ...
  it('folds customHeaders + userAgentEntry into the full connection options', () => {
    const native = buildSeaConnectionOptions(
