 ...
 * finished) wired through `SeaOperationBackend`.
 *
 * The impl-execution feature has not yet wired
 * `DBSQLClient.connect({ useSEA: true })` to dispatch into
 * `SeaBackend`, so this test drives the lifecycle by:
 ...
 *      `Statement` handle.
 *   3. Wrapping that handle in a `SeaOperationBackend` and exercising
 *      its `cancel()` / `close()` / `waitUntilReady()` methods.
 *
 * This mirrors how the eventual `SeaSessionBackend.executeStatement`
 ...
 * `tests/integration/sea/operation-lifecycle-e2e.test.ts`. The
 * existing project structure uses `tests/e2e/**` (with its own
 * `.mocharc.js`), so this file lives under `tests/e2e/sea/` to be
