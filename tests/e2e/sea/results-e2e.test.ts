...
async function fetchProbeRows(useSEA: boolean, secrets: PecoSecrets): Promise<Array<Record<string, unknown>>> {
...
describe('SEA results end-to-end (pecotesting parity gate)', function suite() {
  ...
  it('Thrift and SEA produce byte-identical rows for the probe query (parity gate)', async () => {
    const seaRows = await fetchProbeRows(true, secrets as PecoSecrets);
    const thriftRows = await fetchProbeRows(false, secrets as PecoSecrets);
    expect(seaRows.map(canonical)).to.deep.equal(thriftRows.map(canonical));
  });
});
