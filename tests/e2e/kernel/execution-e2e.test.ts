 ...
 *
 * Walks the full `DBSQLClient` → `KernelBackend` → napi binding → kernel
 * pipeline against a live warehouse over PAT:
 *
 *   1. `connect({ useKernel: true })` selects the kernel backend.
 ...
 *      `KernelOperationBackend` (wraps a napi `Statement`).
 ...
 * "no thrift fallback" indirectly — by selecting `useKernel: true` and
 * exercising the executeStatement path. A proxy that captures
 * `executeStatement` + `GetStatement` wire counts lands in the
 * kernel-integration round; for now we assert that the kernel pipeline
 * itself runs cleanly to completion.
 */
describe('kernel execution end-to-end', function e2eSuite() {
  ...
  it('opens a session, executes SELECT 1, and closes cleanly via kernel backend', async () => {
    ...
    // returned operation. KernelOperationBackend generates a UUIDv4 for
    ...
    // M0 does not yet plumb fetchChunk through the kernel pipeline
    // (kernel-results owns that). We exercise the lifecycle: cancel is a
