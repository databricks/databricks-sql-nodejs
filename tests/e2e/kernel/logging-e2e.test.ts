...
// `DBSQLClient` surface (not the raw binding) so the `KernelBackend` →
...
describe('kernel — unified kernel + driver logging', function unifiedLogging() {
  ...
  it('routes kernel (Rust) logs into the same DBSQLLogger file as driver logs', async () => {
    ...
    await client.connect({
      ...
      ...({ useKernel: true } as InternalConnectionOptions),
    });
