...
/**
 * kernel-auth M0 end-to-end:
 *   1. Construct a DBSQLClient.
 *   2. `connect({ useKernel: true, token })` against pecotesting.
 ...
 * kernel-execution feature's own e2e. This test exists solely to confirm
 ...
 */
describe('kernel-auth e2e — PAT through DBSQLClient ↔ KernelBackend ↔ napi binding', function suite() {
  ...
  it('connects, opens a session, closes the session, closes the client', async () => {
    ...
    const connected = await client.connect({
      ...
      // `useKernel` is an internal opt-in (InternalConnectionOptions), not a
