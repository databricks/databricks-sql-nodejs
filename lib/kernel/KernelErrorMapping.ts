...
export interface KernelMetadata {
  ...
  queryId?: string;
  /**
   * Rich server-error diagnostics from the kernel terminal-error payload. These
   ...
   * connector forwards onto its exceptions — so kernel stays at parity rather
...
/**
 * An `Error` carrying optional kernel-side kernel context. `sqlState` is
...
export function decodeNapiKernelError(err: unknown): Error {
  if (!(err instanceof Error)) {
    return new HiveDriverError(typeof err === 'string' ? err : 'kernel backend: unknown error');
  }
