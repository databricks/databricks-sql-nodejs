 ...
 * `Duration(MICROSECOND)` in the kernel IPC stream. apache-arrow@13's
 ...
 * before any record batch is read, breaking the entire kernel path for any
 ...
 *
 * **Scope on this layer (kernel execution + results, PR 2/3):** the rewrite's
 ...
 * consumer merges, INTERVAL DAY-TIME columns are raw Int64 under kernel.
 *
...
function maybeRewriteSchemaMessage(schemaMessageBytes: Buffer): Buffer | null {
  ...
  //
  // This is a SHARED apache-arrow@13 limitation, NOT a kernel-specific gap:
  // verified against a live warehouse, the Thrift backend throws the
  // identical error for `array(INTERVAL '1' SECOND)` — so kernel matches Thrift
...
function rebuildSchemaWithDurationRewritten(message: Message, fbSchema: FbSchema): Buffer {
  ...
  // emits uncompressed streams for the kernel path per
