 ...
 * rejects every other value; we mirror that surface so the kernel path is a
 * drop-in equivalent:
 *
 *   | TGetInfoType        | Thrift server | kernel (here)        |
 *   |---------------------|---------------|-------------------|
 ...
 * the kernel "undefined → throw" path matches Thrift's effective behaviour rather
 * than narrowing it.
 */

/** Canonical DBMS product name — identical to the Thrift server's value. */
export const KERNEL_DBMS_NAME = 'Spark SQL';

/** Server-name answer — identical to the Thrift server's value. */
export const KERNEL_SERVER_NAME = 'Spark SQL';
 ...
 */
export const KERNEL_DBMS_VERSION = '3.1.1';

/**
 * Synthesize the `TGetInfoValue` for a `getInfo` request on the kernel path.
 ...
 */
export function kernelServerInfoValue(infoType: number): TGetInfoValue | undefined {
  switch (infoType) {
    case TGetInfoType.CLI_SERVER_NAME:
      return new TGetInfoValue({ stringValue: KERNEL_SERVER_NAME });
    case TGetInfoType.CLI_DBMS_NAME:
      return new TGetInfoValue({ stringValue: KERNEL_DBMS_NAME });
    case TGetInfoType.CLI_DBMS_VER:
      return new TGetInfoValue({ stringValue: KERNEL_DBMS_VERSION });
    default:
      return undefined;
  }
}
