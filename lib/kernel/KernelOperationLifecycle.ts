...
/**
 * kernel operation lifecycle helpers (M0).
 *
 ...
 * `KernelOperationBackend`) land independently — at merge time it can
 ...
 *   (`lib/DBSQLOperation.ts:241-259`). For kernel this is a one-shot
 ...
 *   (`lib/DBSQLOperation.ts:265-284`). For kernel this is the napi
 ...
 * tidy.
 */
export function createLifecycleState(): KernelOperationLifecycleState {
...
/**
 * Cancel an in-flight kernel operation.
 *
 ...
 */
export async function kernelCancel(
  state: KernelOperationLifecycleState,
  statement: KernelStatementHandle,
  ...
  context.getLogger().log(LogLevel.debug, `Cancelling kernel operation with id: ${operationId}`);
 ...
 */
export async function kernelClose(
  state: KernelOperationLifecycleState,
  statement: KernelStatementHandle,
  context: IClientContext,
  operationId: string,
  ...
  context.getLogger().log(LogLevel.debug, `Closing kernel operation with id: ${operationId}`);
 ...
 * `getOperationStatus` (`lib/DBSQLOperation.ts:337-391`). For kernel M0,
 ...
 * fetch calls).
 */
export async function kernelFinished(
  state: KernelOperationLifecycleState,
  options?: {
    progress?: boolean;
...
/**
 * Pre-flight check used by fetch* methods on `KernelOperationBackend`.
 ...
 * callers branch on `OperationStateErrorCode` consistently for Thrift and kernel.
 *
 * Exported so impl-results can call it at the top of every fetch
 * call without duplicating the if/throw logic.
 */
export function failIfNotActive(state: KernelOperationLifecycleState): void {
