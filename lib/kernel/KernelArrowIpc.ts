...
export function decodeIpcSchema(ipcBytes: Buffer): Schema<TypeMap> {
  ...
  if (!reader.schema) {
    throw new HiveDriverError('kernel result: Arrow IPC stream carried no schema (empty or truncated payload)');
  }
 ...
 * `lib/result/utils.ts:61-98`) keep working unchanged for the kernel
...
function arrowTypeToTTypeId(field: Field<DataType>): TTypeId {
  ...
  if (DataType.isInt(arrowType)) {
    ...
    // `KernelArrowIpcDurationFix.ts`). Surface them as STRING_TYPE (matching the
