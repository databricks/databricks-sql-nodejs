...
import { KernelNativeBinding, KernelNativeLogRecord } from './KernelNativeLoader';
 ...
 * sink/file.
 */
export function formatKernelLine(record: KernelNativeLogRecord): string {
 ...
 * (`KernelBackend.close()`) to drop the level-change listener; it is a safe no-op
 * when nothing was subscribed.
 */
export function installKernelLogBridge(
  ...
  const callback = (err: Error | null, records: Array<KernelNativeLogRecord>): void => {
    if (err || !records) {
      return;
    }
