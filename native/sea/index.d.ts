...
export declare class Statement {
  ...
  /**
   * Server-side cancel. No-op if already finished or if this
   * `Statement` wraps a metadata `ResultStream` (metadata calls have
   * no in-flight cancellation surface in the kernel today).
   */
  cancel(): Promise<void>
  /**
   * Explicit close. Awaits the server-side close so the JS caller
   * can observe failures. For metadata streams, drops the stream
