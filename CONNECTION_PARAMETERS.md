...
| `enableTokenFederation` / `federationClientId` | `boolean` / `string`                                         |    ✅    |    ⚠️    | `false` / —                                        | On the kernel backend these options are honored only for `static-token`; `federationClientId` is forwarded only when `enableTokenFederation` is `true`. The Thrift backend also supports them for `token-provider` and `external-token`.                                                                                                              |
